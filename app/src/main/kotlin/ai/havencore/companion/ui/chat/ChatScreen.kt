...
fun ChatScreen(
    ...
    var showAssistPrompt by remember { mutableStateOf(false) }
    LaunchedEffect(Unit) {
        val seen = vm.assistPromptSeenFlow.first()
        if (!seen && !DefaultAssistantHelper.isHeld(ctx)) {
    ...
    if (showAssistPrompt) {
        AlertDialog(
            onDismissRequest = {
                vm.markAssistPromptSeen()
                showAssistPrompt = false
            },
            title = { Text("Set HavenCore as your default assistant?") },
            text = {
                Text(
                    "Long-press home or power to talk to HavenCore from any " +
                        "screen, including the lockscreen.",
                )
            },
            confirmButton = {
                TextButton(onClick = {
